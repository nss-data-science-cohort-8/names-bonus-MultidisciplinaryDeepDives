...
FROM names 
WHERE name NOT SIMILAR TO '%(a|e|i|o|u|y)%' AND 
		name NOT SIMILAR TO '(A|E|I|O|U|Y)%'
      ;
...
SELECT DISTINCT name
FROM names
WHERE regexp_like(LOWER(name), '([a-zA-Z])\1')
;


SELECT DISTINCT name
FROM names
WHERE regexp_like(LOWER(name), '([a-zA-Z])\1\1')
;

-- Side Note: the \1 is referencing the content inside of the parenthesis 
